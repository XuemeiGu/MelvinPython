...
def PrintOptions(expr,printflag):## returen setup list     
    if printflag==1:
        print(expr)
...
'''
good examples: 
SRV:[3, 3, 2]; Setup: OAMHolo(OAMHolo(OAMHolo(LI(OAMHolo(Reflection(XXX,e),a,3),b,c),d,5),a,2),e,7); FF1:[4, 5]
SRV:[3, 3, 2]; Setup: OAMHolo(OAMHolo(OAMHolo(LI(OAMHolo(Reflection(XXX,e),a,3),b,c),d,5),a,2),e,7); FF1:[5, 6]
SRV:[3, 3, 2]; Setup: OAMHolo(DP(OAMHolo(LI(OAMHolo(OAMHolo(XXX,b,2),f,8),a,d),f,3),b),c,2); FF1:[0, 1]
SRV:[3, 3, 2]; Setup: OAMHolo(DP(OAMHolo(LI(OAMHolo(OAMHolo(XXX,b,2),f,8),a,d),f,3),b),c,2); FF1:[0, -1]
...
setupStr='OAMHolo(OAMHolo(OAMHolo(LI(OAMHolo(Reflection(XXX,e),a,3),b,c),d,5),a,2),e,7)'
triggers=[4,5]

#setupStr='OAMHolo(OAMHolo(BS(BS(OAMHolo(OAMHolo(OAMHolo(OAMHolo(DCState,c,-3),a,3),f,-5),c,2),a,c),a,b),b,-3),e,-4)'
#triggers=[1]
...
PrintOptions('-------give the output of one experimental setup-------',PrintFlag)
PrintOptions('     ',PrintFlag)
       
output_state=outputState
ReducedVV=Trigger(output_state,FF1,triggers) 
...
PrintOptions('TermofState:  '+str(TermofState)+' CoeffsEqual?  '+str(CoeffsEqual),PrintFlag)
PrintOptions('     ',PrintFlag)
...
if DimVecEncode[2]>1 and InfoLetter=='_OKTerms_MaxEnt' :  #the index starts from 0
   ...
   if DimVec[2]>1 and TermofState== DimVec[0]:  ## check maximun entangled states
       ...
       ReducedVVHO=Trigger(highoutputState,FF1,triggers)
       ...
       if ComplementDC2 != []:
            for ii in range(len(ComplementDC2)):
                ReducedVVHO=ReducedVVHO.replace(FF2[ComplementDC2[ii]],0, map=False, simultaneous=True, exact=False)
       if ComplementDC3 != []:               
            for ii in range(len(ComplementDC3)):
                ReducedVVHO=ReducedVVHO.replace(FF3[ComplementDC3[ii]],0, map=False, simultaneous=True, exact=False) 
       if ComplementDC4 != []:
            for ii in range(len(ComplementDC4)):
                ReducedVVHO=ReducedVVHO.replace(FF4[ComplementDC4[ii]],0, map=False, simultaneous=True, exact=False) 
           
       PrintOptions('Large_SPDC Output State for the setup: '+str(ReducedVVHO),PrintFlag)
       PrintOptions('   ',PrintFlag)
       if ReducedVV==ReducedVVHO:        
#            PrintOptions('This is a good SRV:'+str(DimVec),PrintFlag)
            print('This is a good SRV:',str(DimVec))
            PrintOptions('     ',PrintFlag)
       else :
#            PrintOptions('This is a bad SRV beacuse of high-order OAM',PrintFlag)
